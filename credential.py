...
class BceCredentials(object):
    def __init__(self, access_token, secret_token):
        ...
        self.RESERVED_CHAR_SET = set(string.ascii_letters + string.digits + '.~-_')
        self.get_normalized_char_list()

    def get_normalized_char(self, i):
        char = chr(i)
        if char in self.RESERVED_CHAR_SET:
            return char
        else:
            return '%%%02X' % i

    def get_normalized_char_list(self):
        """
        根据RFC 3986，除了：
          1.大小写英文字符
          2.阿拉伯数字
        ...
        以外都要编码
        """
        self.NORMALIZED_CHAR_LIST = [self.get_normalized_char(i) for i in range(256)]
        return self.NORMALIZED_CHAR_LIST
    ...
    def normalize_string(self, in_str, encoding_slash=True):
        ...
        if encoding_slash:
            encode_f = lambda c: self.NORMALIZED_CHAR_LIST[ord(c)]
        else:
            # 仅仅在生成规范URI时。不需要对斜杠'/'进行编码
            encode_f = lambda c: self.NORMALIZED_CHAR_LIST[ord(c)] if c != '/' else c
    ...
    def get_canonical_querystring(self, params):
        ...
        result = ['%s=%s' % (k, self.normalize_string(v)) for k, v in params.items() if k.lower != AUTHORIZATION]
    ...
    def get_canonical_headers(self, headers, headers_to_sign=None):
        ...
        for k, v in map(f, headers.iteritems()):
            ...
            if k.startswith(BCE_PREFIX) or k in headers_to_sign:
                result.append("%s:%s" % (self.normalize_string(k), self.normalize_string(v)))
    ...
    def sign(self, http_method, path, headers, params,
        ...
        sign_key_info = 'bce-auth-v1/%s/%s/%d' % (
            self.access_token,
            self.get_canonical_time(timestamp),
